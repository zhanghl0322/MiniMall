...
class PondForm extends ApiModel
{
    ...
    public function index()
    {
        ...
        $lottery_user = LotteryUser::find()
            ->where(['store_id' => $this->store_id, 'user_id' => $this->user_id])
            ->andWhere(['>', 'end_time', $start_time])
            ->all();
        if ($log >= $setting['oppty']) {
            \Yii::warning("==是否日志进入......==",'info');
            return [
                'code' => 1,
                'data' => [
                    'list' => $list,
                    'oppty' => (int)$lottery_user[0]['oppty'],
                    'images' => $images,
                    'time' => true,
                    'register' => $setting['deplete_register'],
                    'integral' => true,
            ...
            \Yii::warning("==不是日志记录......==",'info');
            return [
                'code' => 0,
                'data' => [
                    'list' => $list,
                    'oppty' => ($setting['oppty'] - $log)+(int)$lottery_user[0]['oppty'],
                    'images' => $images,
                    'time' => true,
                    'register' => $setting['deplete_register'],
                    'integral' => true,
    ...
    public function lottery()
    {
        ...
        $log = PondLog::find()
            ...
            ->andWhere(['<', 'create_time', $end_time])
            ->count();
        if ($log >=$list['oppty']&&$lotteryUser->oppty==0) {
            return [
                'code' => 1,
                'msg' => '机会已用完'
            ];
        ...
        if ($pondLog->save()) {
            ...
            if($log >= $setting['oppty'])
            {
                $lotteryUser->save();
                $array = [
                    'oppty' =>$lotteryUser->oppty,
                    'id' => $id,
                    'p_id' => $pondLog->id,
                    'type'=> $form->type    // TODO #1.红包2.优惠卷3.积分4.实物.5.谢谢参与
                ];
                return [
                    'code' => 0,
                    'msg' => $msg,
                ...
                $array = [
                    'oppty' => $list['oppty'] - $log + $lotteryUser->oppty,
                    'id' => $id,
                    'p_id' => $pondLog->id,
                    'type'=> $form->type    // TODO #1.红包2.优惠卷3.积分4.实物.5.谢谢参与
                ];
