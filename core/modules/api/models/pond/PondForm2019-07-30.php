...
class PondForm extends ApiModel
{
    ...
    public function index()
    {
        ...
        //TODO 新增查询用户抽奖次数  2019年7月24日09:57:43 Allon  用户获取的抽奖次数 + 平台统一发放的抽奖次数
        $lottery_user = LotteryUser::find(['user_id' => $this->user_id])->andWhere(['>', 'end_time', time()])->all();
        $user_count=$lottery_user[0]['oppty'];
        $setting = PondSetting::findOne(['store_id' => $this->store_id]);
        $oppty = $setting['oppty'];  //系统设定次数+用户获取的次数
        ...
        if ($setting['deplete_register'] > 0) {
            ...
            if ($user->integral < $setting['deplete_register']) {
                return [
                    ...
                    'data' => [
                        'list' => $list,
                        'oppty' => $setting['oppty'] - $log+$user_count,
                        'time' => true,
                        'register' => $setting['deplete_register'],
                        'integral' => false,
                    ],
    ...
    public function lottery()
    {
        ...
        $lotteryUser =LotteryUser::findOne(['user_id' => $this->user_id, 'store_id' => $this->store_id]);
        $lotteryUser->oppty -=1;
       // \Yii::warning("==测试成绩次数......==".$lotteryUser->oppty ,'info');
        ...
        if ($list->deplete_register > 0) {
            ...
            //TODO 更新用户抽奖剩余次数
            if (!$lotteryUser->save()) {
                //\Yii::warning("==测试成绩次数失败......==".$lotteryUser->oppty ,'info');
                $t->rollBack();
                return $this->getErrorResponse($lotteryUser);
            }

        }
        if ($pondLog->save()) {
            $t->commit();
            $num = $lotteryUser->oppty+$list['oppty'];
            if($log >= $list['oppty'])
            {
                $num = $lotteryUser->oppty;
                $lotteryUser->save();
            }
           // \Yii::warning($list['oppty']."==测试成绩次数成功......==".$lotteryUser->oppty ,'info');
//            //TODO 更新用户抽奖剩余次数
            $array = [
                'oppty' =>$num,
                'id' => $id,
    ...
    public function myqrcode()
    {
        ...
        $wxapp_qrcode_file_res = $this->getQrcode($scene, 240, "pages/coupon/coupon");
