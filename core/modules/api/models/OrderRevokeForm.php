...
class OrderRevokeForm extends ApiModel
{
    ...
    public function save()
    {
        ...
        //==========================================================
        //已付款就退款
        //TODO 账户余额+微信支付 合并支付  退款默认统一退至账户余额  2019年8月20日11:02:12
        //账户余额+微信支付
        \Yii::warning($order->order_no.'账户余额+微信支付退款'.$order->pay_type,'info');
        if ($order->is_pay == 1 && $order->pay_type == 4) {
            if ($order->pay_price > 0) {
                \Yii::warning($order->order_no.'账户余额+微信支付退款第二步'.$order->pay_type,'info');
                $user->money += floatval($order->pay_price);//改造以后使用express_price_1 字段记录原订单总金额
        ...
        //TODO 余额支付  退款 Allon  2019年8月16日10:11:13
        //余额支付 退换余额
        ...
        if ($order->is_pay == 1 && $order->pay_type == 1) {
            if ($order->pay_price > 0) {
                \Yii::warning($order->order_no."==微信支付退款第一步==" .$order->pay_price,'info');
     ...
     * @return array|bool
     */
    //TODO 退款  Allon  2019年8月15日11:11:43
