...
class IntegralForm extends ApiModel
{
    ...
    {
        //TODO 新增type类型 99 为积分兑换应对方案  2019年7月22日11:39:40  Allon
        if (!$this->validate()) {
        ...
        foreach ($list as $k => &$v) {
            ...
            switch ($v['type']) {
                ...
                case 99:
                    /* @var MsOrder $order */
                    $newItem['content'] = "积分抽奖";
                    $newItem['content_1'] = "扣除积分";
                    break;
                case 100:
                    //TODO 2019年7月23日16:33:45  由于临时性启用积分洗车描述字段  所有 暂时使用register_time 存储
                    /* @var MsOrder $order */
                    $newItem['content'] = $v->register_time;
                    $newItem['content_1'] = "平台赠送积分";
                    break;
                default:
                    if ($v->integral >= 0) {
