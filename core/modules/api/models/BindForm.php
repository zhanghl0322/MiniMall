...
class BindForm extends ApiModel
{
    ...
    /**
     * 分销绑定用户上下级 关系  2019年10月29日11:38:35
     * @return array
     * 绑定上下级关系
     */
    public function save()
    {
        ...
        if ($setting->condition !== $this->condition) {
            \Yii::warning($this->user_id.'***************WMZCWXXTJ****************'.$this->condition,'info');
        ...
        if (!$exit) {
            \Yii::warning($this->parent_id.'***************Yes Exit****************'.$exit,'info');
            \Yii::warning('不是分销商');
            return [
                'code' => 1,
                'msg' => '不是分销商'
        ...
        \Yii::warning($this->parent_id.'***************No Exit****************'.$exit,'info');
        //判断父级id是否在三级分销链中
