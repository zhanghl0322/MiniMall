...
class DetailForm extends ApiModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['id'], 'integer'],
            [['order_type'], 'string'],
            [['order_type'], 'in', 'range' => ['s', 'ms', 'pt', 'yy', 's_re', 'ms_re', 'pt_re', 'yy_re', 'r', 'log', 'pond', 'scratch','user_action_log']],
        ];
    }
    ...
    public function search()
    {
        ...
        elseif ($this->order_type == 'user_action_log') {
            //TODO 新增 用户动作日志 含订单支付 退款
            $data = $this->getUserActionLog();
        }
        else {
            return [
                'code' => 1,
    ...
    // 用户消费抵扣记录  2019年8月27日09:56:45
    public function getUserActionLog()
    {
        $new_list = [];
        $list = UserAccountLog::find()->alias('ual')->where([
            'ual.order_type' => [3,4]
        ])->leftJoin(['o' => Order::tableName()], 'o.id=ual.order_id')
            ->andWhere(['o.pay_type' => [3,4], 'ual.id' => $this->id])->select(['ual.price as refund_price', 'o.addtime','o.order_no'])->asArray()->one();
    ...
    public function getOrder()
    {
        ...
        $list = Order::find()->where([
            'store_id' => $this->store_id, 'id' => $this->id, 'is_pay' => 1, 'pay_type' => [3,4]
        ])->asArray()->one();
    ...
    public function getOrderRefund()
    {
        ...
        $list = OrderRefund::find()->alias('ore')->where([
            ...
            ->andWhere(['o.pay_type' => [3,4], 'ore.id' => $this->id])->select(['ore.*', 'o.order_no'])->asArray()->one();
        $new_list['time'] = date('Y-m-d H:i:s', $list['addtime']);
        $new_list['pay_price'] = '+' . $list['refund_price'];
        $new_list['send_price'] = $list['send_price'];
        $new_list['content'] = "消费-商城订单退款";
