...
class RecordForm extends ApiModel
{
    ...
    //TODO 用户中心余额记录  2019年8月10日17:48:06
    public function search()
    {
        ...
        foreach ($list as $index => $value) {
            ...
            elseif(in_array($value['order_type'], ['user_action_log'])){
                //TODO 新增用户账户余额明细  2019年8月23日14:25:16 Allon
                $list[$index]['flag'] = 0;
                $list[$index]['price'] = '+' . (floatval($value['pay_price']) + floatval($value['send_price']));
               // $list[$index]['content'] = "刮刮卡获得";
            }
    ...
    public function getSql()
    {
        ...
        $user_action_Table = UserAccountLog::tableName();//用户动作表
        $query_r = "(
            SELECT
            'r' AS order_type,
            id,
        ...
        //TODO 用户动作Log、包含订单扣除、退回记录  2019年8月23日10:27:43
        //订单类型 0--充值 1--商城订单 2--秒杀订单 3--拼团订单 4--商城订单退款 5--秒杀订单退款 6--拼团订单退款
        $query_user_action_log = "(
            SELECT
            'user_action_log' AS order_type,ual.id,ual.addtime,ual.price AS pay_price,0 AS send_price,ual.desc AS  content
            FROM {$user_action_Table} AS ual
            WHERE ual.order_type IN (4)          
            AND ual.user_id = {$this->user->id}
        )";
     \Yii::warning($logTable.'====记录执行Sql语句===='.$query_log,'info');
        $pondLog = "(
            SELECT
            'pond' AS order_type,
            p.id,
