...
class OrderListForm extends ApiModel
{
    ...
    public function search()
    {
       // $order_count =$this->getDataCount($this->store_id, $this->user_id);
        //TODO 新增视图并单查询
        $sql="SELECT  * FROM v_order_list WHERE  user_id=".$this->user_id;
        $order_list = \Yii::$app->db->createCommand($sql)->queryAll();
        $order_detail_all_list= \Yii::$app->db->createCommand('SELECT  * FROM v_order_detail_list')->queryAll();
        if (!$this->validate()) {
...
//                    'goods_pic' => $goods->getGoodsPic(0)->pic_url,
        ...
        foreach ($order_list as $orders) {
            $Order_arr = $orders['order_id'];
        }
        \Yii::warning('查询全部数据Order'.var_dump($Order_arr),'info');

        //TODO For 查询并单信息
        foreach ($order_list as $order_all) {
            $goods_all_list = [];
//            $order_detail_all_list =\Yii::$app->db->createCommand('SELECT  * FROM v_order_detail_list where order_id='.$order_all['id'])->queryAll();
            //TODO 输出参数定义
//            foreach ($order_detail_all_list as $order_detail_all) {
//               //TODO 输出视图拼单
//                if (!$order_detail_all) {
//                    continue;
//                }
...
//            }
        ...
        return [
            ...
            'data' => [
                ...
                'list' => $new_list,
                'test_list' => $goods_all_list,
                'pay_type_list' => $pay_type_list,
                'order_list' => $order_list,
                'order_detail_list' => $order_detail_all_list,
                'order_count' => $count
            ],
        ];
    }
...
//                    'goods_pic' => $goods->getGoodsPic(0)->pic_url,
    ...
    public  function getDataCount($store_id, $user_id)
    {
        $form = new OrderListForm();
        $form->limit = 1;
        $form->store_id = $store_id;
