...
class OrderPayDataForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        if ($this->order_id) { //单个订单付款
            ...
            //TODO 将余额支付跟在线支付业务合并 Allon  2019年8月14日09:51:04    pay_type =0  为 抽奖构建的订单
            if ($this->pay_type == 'WECHAT_PAY'||$this->pay_type == 'BALANCE_PAY') {
                if ($this->pay_type == 'BALANCE_PAY') {
                 \Yii::warning($this->pay_type."==订单支付总金额BALANCE_PAY==".$this->order->pay_price ,'info');
                }
                \Yii::warning($this->pay_type."==订单支付总金额WECHAT_PAY==".$this->order->pay_price ,'info');
                ...
                if (isset($res['code']) && $res['code'] == 1) {
                    \Yii::warning($this->pay_type."==账户余额支付请求成功NO1==".$res['code'] ,'info');
                    return $res;
                }
                \Yii::warning($this->pay_type."==账户余额支付请求成功NO2==".$res['code'] ,'info');
                //记录prepay_id发送模板消息用到
                FormId::addFormId([
                    'store_id' => $this->store_id,
                ...
                \Yii::warning($this->pay_type."==账户余额支付请求成功NO3==".$res['code'] ,'info');
                $pay_data = [
                ...
                \Yii::warning($this->pay_type."==账户余额支付请求成功NO4==". $pay_data['paySign']  ,'info');

                \Yii::warning($this->pay_type."==账户余额支付请求成功NO5==".  $this->order->pay_type ,'info');
                return [
                    'code' => 0,
                    'msg' => 'success',
