...
//        $form1->user_id = \Yii::$app->user->id;
//        $form1->ExchangeDrawSave();
    ...
    //TODO 积分兑换抽奖次数  每次例 ： 20 积分兑换一次抽奖机会
    public function actionExchangeDraw()
    {
        $today = \Yii::$app->request->get('today');
        $form = new RegisterForm();
        $form->store_id = $this->store->id;
        $form->user_id = \Yii::$app->user->id;
        $form->register_time = $today;
        return new \app\hejiang\BaseApiResponse($form->ExchangeDrawSave());
    }
}
