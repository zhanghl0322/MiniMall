...
class DefaultController extends Controller
{
    ...
    {
        $cache = \Yii::$app->cache;
        $params =  \Yii::$app->params['cacheKey.index'];
        $key=$params[0];
        $time=$params[1];
        if ($cache->exists($key)) {
    ...
    public function actionGoods()
    {
        ...
        $params =  \Yii::$app->params['cacheKey.goods'];
        $request=\Yii::$app->request->get();
        $key=$params[0].$request["id"];
        $time=$params[1];
        ...
        if ($cache->exists($key) && $request["id"]  ) {
    ...
    public function actionCommentList()
    {
        ...
        $cache = \Yii::$app->cache;
        $params =  \Yii::$app->params['cacheKey.commentList'];
        $request=\Yii::$app->request->get();
        $key=$params[0]."id".$request["goods_id"]."p".$request["page"];
        $time=$params[1];

        if ($cache->exists($key) && $request["goods_id"]) {
            return $cache->get($key);
        }
