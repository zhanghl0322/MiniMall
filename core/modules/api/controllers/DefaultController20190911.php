...
class DefaultController extends Controller
{
    ...
    {
        \Yii::warning("==首页参数挂载加载......==" ,'info');
        $form = new IndexForm();
        $form->store_id = $this->store->id;
        $form->_platform = \Yii::$app->request->get('_platform');
        $form->page_id = \Yii::$app->request->get('page_id');

        //TODO 迁移模板逾期代付通知  2019年7月17日16:03:27
//        if($form->page_id==-1)
////        {
////            //TODO 迁移模板逾期代付通知  2019年7月17日16:03:27
////            $order_form = new \app\modules\api\models\order\OrderSubmitForm();
////            $form_list= $order_form->getSendFormIdList();
////            for ($i = 0; $i < count($form_list); $i++) {
////                $name = $form_list[$i]['name'];//商品名称
////                $total_price = $form_list[$i]['total_price'];//总价
////                $form_id = $form_list[$i]['form_id'];//表单Id
////                $wechat_open_id = $form_list[$i]['wechat_open_id'];//微信Open_Id
////                $order_no=$form_list[$i]['order_no'];//订单编号
////                $this->NonPaymentMsg($name,$total_price,$form_id,$wechat_open_id,$order_no);
////            }
////        }
        return $form->search();
    }
     ...
     * @return array
     */
    public function NonPaymentMsg($name, $total_price, $form_id, $wechat_open_id,$order_no)
    {
        $access_token = $this->wechat->getAccessToken();
        \Yii::warning("发送待支付模板消息".$access_token ,'info');
        try {
            $data = [
                'touser' => 'ohXJd5TSaivcYRMWRDFdUlwwtxkQ',//ohXJd5QFdDnjTasfN7k8jBuD-eHw 于   ohXJd5RjpwBp8-QwLBnpg6CXB1-A Z
                'template_id' => 'ydx-ep98BVniTp63G8YjWDUzREGa11kEXvKKrFVUmbQ',
                'form_id' => '11fb209e262b4738893e593c691afdb4',
                'page' => 'pages/order/order?status=0',//点击进去跳转界面
                'data' => [
                    'keyword1' => [
                    ...
                    'keyword4' => [
                        'value' => '您有一笔订单未支付，30分钟后未付款将自动取消，请尽快支付。',
                        'color' => '#333333',
                    ],
                ],
            ];
            ...
            if (!empty($res['errcode']) && $res['errcode'] != 0) {
                \Yii::warning("模板消息发送失败". json_encode($res, JSON_UNESCAPED_UNICODE) ,'info');
            }
            else
            {
                $sql = "UPDATE hjmall_form_id SET send_count=1 WHERE TYPE='form_id' AND order_no ='{$order_no}'";
                $order_list = \Yii::$app->db->createCommand($sql)->queryAll();//更新已执行的Send_Count 数、用于后续过滤
                return $order_list;
                \Yii::warning("模板消息发送成功" ,'info');
            }
        } catch (\Exception $e) {
