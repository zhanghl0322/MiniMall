...
use app\models\User;
use app\modules\mch\models\DiscountActivitiesEditForm;
use app\modules\mch\models\CouponSendForm;
use app\modules\mch\models\full\Goodsfullcsv;

//TODO 新增折扣管理 2019年9月10日10:03:47
class DiscountController extends Controller
{
    //优惠券列表
    public function actionIndex()
    {
        $list = DiscountActivities::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->orderBy('sort ASC')->all();
    ...
    {
        \Yii::warning('测试数据是否进入满减编辑页面','info');
        ...
        if (!$model) {
            $model = new DiscountActivities();
        }
        //====================更新活动状态保证只有一个活动进行=========================
        //DiscountActivities::updateAll(['is_join' => 1], ['coupon_id' => $model->id]);
        //========================================================
        if (\Yii::$app->request->isPost) {
            $form = new DiscountActivitiesEditForm();
            $form->attributes = \Yii::$app->request->post();
            $form->store_id = $this->store->id;
            $form->discount_activities = $model;
            return $form->save();
        } else {
    ...
    public function actionDeleteCat()
    {
        ...
        $coupon = DiscountActivities::findOne([
            'id' => $cat_id['discount_id'],
            'store_id' => $this->store->id,
            'is_delete' => 0,
        ]);
        if (!$coupon) {
...
//    查找商品
    ...
    // 满减商品CSV上传
    //TODO 新增商品评论CVS 上传 2019年9月27日10:49:49
    public function actionGoodsExcel()
    {
        \Yii::warning("==满减Excel导入==",'info');
        if (\Yii::$app->request->isPost) {
            $form = new Goodsfullcsv();
            $form->attributes = \Yii::$app->request->post();
            $form->store_id = $this->store->id;
            $res = $form->search();
            return $res;
        }
        return $this->render('goods-excel');
    }
}
