...
use app\modules\mch\models\user\coupon\IndexCouponForm;//TODO 引入自定义表单
...
use app\modules\mch\models\CouponExportList;//TODO 新优惠券导出 2019年9月9日09:45:39
...
class UserController extends Controller
{
     ...
     * @return string
     */
    public function actionCouponSendList()
    {
        \Yii::warning("==是否券台账界面测试......==",'info');
        $userExport = new CouponExportList();
        $exportList = $userExport->getList(1);

        $model = new IndexCouponForm();
        $model->date_start=\Yii::$app->request->get('date_start');
        $model->date_end=\Yii::$app->request->get('date_end');
        $model->is_use=\Yii::$app->request->get('platform');
        $model->coupon_type=\Yii::$app->request->get('coupon_type');
        $model->coupon_name=\Yii::$app->request->get('coupon_name');
        $model->attributes = \Yii::$app->request->post();
        $res = $model->getCouponSendList();

        return $this->render('coupon-send-list', [
            'list' => $res['list'],
            'pagination' => $res['pagination'],
            'exportList' => \Yii::$app->serializer->encode($exportList)
        ]);
    ...
    //TODO 新增用户券台账界面 2019年7月31日11:40:09  Allon
    /**
     * 优惠券台账列表
     * @return string
     */
...
//        $model->is_use=\Yii::$app->request->get('platform');
//        $model->coupon_type=\Yii::$app->request->get('coupon_type');
//        $model->flag=\Yii::$app->request->get('EXPORT');
//        $res = $model->getCouponSendList();
//        return $this->render('coupon-send-list', [
//            'list' => $res['list'],
//            'pagination' => $res['pagination'],
...
//    }
    ...
    {
        \Yii::warning('===进入充值金额==','info');
        if (\Yii::$app->request->isPost) {
            $form = new \app\modules\mch\models\UserRechargeForm();

            $form->ExchangeMoney();//测试调用  2019年10月25日14:11:503
