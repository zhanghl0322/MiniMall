...
//    查找商品
    public function actionSearchGoods($keyword)
    {
        ...
        $list = $query->orderBy('c.name')->limit(100)->asArray()->select('id,cover_pic,name')->all();
        return [
            'code' => 0,
            'msg' => 'success',
            'data' => (object)[
    ...
    //查找用户
    public function actionSearchUser($keyword,$keyword_name)
    {
        $keyword = trim($keyword);
...
//        ]);,'u.id' => $keyword
        \Yii::warning($keyword.'$keyword_name'.$keyword_name,'info');
        ...
        $list = $query->orderBy('u.nickname')->limit(150)->asArray()->select('id,nickname,avatar_url')->all();
        return [
            'code' => 0,
            'msg' => 'success',
            'data' => (object)[
