...
use app\modules\mch\models\goods\Taobaoexcel;//TODO 新增Excel 导入评论功能
...
class GoodsController extends Controller
{
    ...
    public function actionGoods($keyword = null, $status = null)
    {
        ...
        return $this->render('goods', [
            ...
            'exportList' => \Yii::$app->serializer->encode($exportList) //TODO 新增商品导出Excel 2019年9月26日14:51:08
        ]);
    }
    ...
    public function actionGoodsEdit($id = 0)
    {
        ...
        if (\Yii::$app->request->isPost) {
            ...
            \Yii::warning('测试编辑热销============'.$model['is_best'] , 'info');
    ...
    {
        \Yii::warning('----进入设置订单界面----','info');
        $form = new SetGoodsPriceForm();
        $form->attributes = \Yii::$app->request->get();
        $form->store_id = $this->store->id;
        return $form->save();
    }
    ...
    // 商品评论CSV上传
    //TODO 新增商品评论CVS 上传
    public function actionTaobaoExcel()
    {
        \Yii::warning("==Excel导入==",'info');
        if (\Yii::$app->request->isPost) {
            $form = new Taobaoexcel();
            $form->attributes = \Yii::$app->request->post();
            $form->store_id = $this->store->id;
            $res = $form->search();
            return $res;
        }
        return $this->render('taobao-excel');
    }


    //TODO 新增商品浏览PV列表台账  2019年9月24日15:08:57
    public function actionGoodsPvList()
    {
        $form = new GoodsPvListForm();
        $form->store_id = $this->store->id;
        $form->attributes = \Yii::$app->request->get();
        $form->page=\Yii::$app->request->get('page');//获取地址栏传递的参数
        $form->date_start=\Yii::$app->request->get('date_start');//获取地址栏传递的参数
        $form->date_end=\Yii::$app->request->get('date_end');//获取地址栏传递的参数
        $arr = $form->search();
        return $this->render('goods-pv-list', [
            'list' => $arr['list'],
    ...
    //TODO 新增商品浏览UV列表台账  2019年9月26日11:30:39
    public function actionGoodsUvList()
    {
        $form = new GoodsPvListForm();
        $form->store_id = $this->store->id;
        $form->attributes = \Yii::$app->request->get();
        ...
        $form->date_end=\Yii::$app->request->get('date_end');//获取地址栏传递的参数
        $arr = $form->searchUv();
        return $this->render('goods-uv-list', [
            'list' => $arr['list'],
            'row_count'=>$arr['row_count'],
            'pagination' => $arr['pagination'],
        ]);
    ...
    //TODO 设置是否店长推荐  2019-07-10 16点40分
    /**
     * 设置是否店长推荐
     * @param $id
     * @param $status
     * @return array
     */
    public function actionSetBest($id, $status)
    {
        $goods= Goods::findOne([
            'id' => $id,
            'store_id' => $this->store->id,
    ...
    //TODO 设置是否前端显示  2019年8月7日16:34:00
    /**
     * 设置是否前端显示
     * @param $id
     * @param $status
     * @return array
     */
    public function actionSetShow($id, $status)
    {
        $goods= Goods::findOne([
            'id' => $id,
            'store_id' => $this->store->id,
