        ...
        <table class="table table-bordered bg-white table-hover">
            ...
            <?php foreach ($list as $index => $goods) : ?>
                <tr>
                    ...
                    <td class="nowrap text-danger">
                        <input class="form-control form-control-sm set-goods-price" value="<?= $goods->price ?>"
                               data-id="<?= $goods->id ?>" >
                        元</td>
                    <td class="nowrap text-danger" style="display: none"><?= $goods->price ?>元</td>
...
//        var more = $('input[name^="integral[more]"]').val();
    ...
    //TODO 新增设置商户列表的产品价格   2016-06-27  Allon  09点29分  set-goods-price
    $(document).on('change', '.set-goods-price', function () {
        var input = $(this);
        var id = input.attr('data-id');
        ...
        $.loading();
        $.ajax({
            url: '<?=Yii::$app->urlManager->createUrl(['mch/goods/set-price'])?>',
            data: {
                goods_id: id,
                price: price,
            },
