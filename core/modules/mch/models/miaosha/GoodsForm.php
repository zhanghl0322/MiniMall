...
class GoodsForm extends MchModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['store_id', 'freight','mch_id', 'use_attr', 'is_discount', 'coupon', 'attr_setting_type'], 'integer'],
...
//            ->andWhere(['is_delete' => 0, 'store_id' => $this->store_id]);
        ...
        $query = MsGoods::find()
            ->alias('g')
            ->andWhere(['g.is_delete' => 0, 'g.store_id' =>  $this->store_id])
            ->leftJoin('{{%mch}} c', 'g.mch_id=c.id')
            ->select([
                'g.*', 'c.name as mchname'
            ]);
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            $goods->mch_id=$this->mch_id;//入驻商户id
            if ($goods->save()) {
