...
class OrderListForm extends MchModel
{
    ...
    //TODO 新增秒杀订单列表、展示商户附属订单
    public function searchMiao()
    {
        \Yii::warning('测试Log'. $this->mch_id,'info');
        if (!$this->validate()) {
            return $this->errorResponse;
        }
        $query = MsOrder::find()->alias('o')->where([
            ...
            ->leftJoin(['u' => User::tableName()], 'u.id=o.user_id')
            ->leftJoin(['g' => MsGoods::tableName()], 'g.id=o.goods_id');
          //  ->leftJoin(['goods' => Goods ::tableName()], 'goods.id=o.goods_id  and  goods.mch_id='.$this->mch_id );//过滤商户
