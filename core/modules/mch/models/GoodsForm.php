...
class GoodsForm extends MchModel
{
    ...
    public $is_best; // TODO 补充 是否店长推荐  标识字段  2019-07-10 14点27分
    public $is_show; // TODO 补充 是否展示  标识字段  2019年8月7日16:45:35

    public $supplier_url; // TODO 补充 供应商url
    public $supplier_name; // TODO 补充 供应商名称
    //TODO Yii2 rules 的验证规则 新补充的字段一定要定义上去
    /**
     * @return array
     */
    ...
    {
        return [
            [['name', 'service', 'unit', 'goods_no','supplier_name'], 'trim'],
            [['store_id', 'name', 'price', 'cat_id', 'detail', 'goods_pic_list', 'cover_pic', 'attr_setting_type'], 'required'],
            [['store_id', 'share_type', 'quick_purchase', 'hot_cakes', 'is_level', 'is_negotiable','is_best','is_show'], 'integer'],
            [['price', 'original_price'], 'number', 'min' => 0.01, 'max' => 99999999],
            [['detail', 'service', 'cover_pic', 'video_url', 'goods_no','supplier_url'], 'string'],
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            $goods->is_best=$_this_attributes['is_best'];//TODO 新增是否店长添加 热门优选商品  2019-07-10 14点32分
            $goods->is_show=$_this_attributes['is_show'];//TODO 新增是否店长添加 热门优选商品  2019-07-10 14点32分
