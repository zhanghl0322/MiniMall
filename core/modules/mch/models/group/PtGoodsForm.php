...
class PtGoodsForm extends MchModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['store_id', 'cat_id', 'freight', 'limit_time', 'is_only', 'is_more', 'type', 'use_attr', 'share_type', 'attr_setting_type', 'is_level','mch_id'], 'integer'],
    ...
    public function getList($store_id)
    {
        $query = PtGoods::find()
            ...
            ->select([
                ...
                'g.virtual_sales', 'g.cover_pic', 'c.name AS cname','g.attr','g.mch_id'
            ])->with('detail');
        ...
        foreach ($list as $key => $goods) {
            ...
            $list[$key]['mch_name'] =   $this->getMchName($list[$key]['mch_id']);
        }
    ...
    {
        $mch_model = Mch::findOne(['id' => $mch_id,  'is_delete' => 0]);
        return $mch_model->name;
    }
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            $goods->mch_id = $this->mch_id;//TODO 设置附属商户值填充
            if ($goods->save()) {
