...
class ExportList
{
    ...
    //TODO  新增导出Order数据模板  2019-06-25  16点43分
    ...
    public function dataTransform($data)
    {
        ...
        foreach ($data as $datum) {
            ...
            $newItem['pay_desc'] =  $datum->pay_desc ? $datum['pay_desc'] : '';
            $newItem['full_reduction_price'] = $datum->full_reduction_price ? $datum['full_reduction_price'] : '';
...
//        Export::order_3($newList, $newFields);
    ...
    function dataTransform_new($query)
    {
        ...
        $count = $query->count();
        for ($i = 0; $i < $count; $i += $limit) {
            $query->select('o.*,m.name as mch_name');
            if ($this->order_type == 0) {
...
// 获取需要导出的字段
    ...
    function data_new(&$data)
    {
        ...
        foreach ($data as &$datum) {
            ...
            $newItem['total_price'] = $datum->total_price;
            $newItem['pay_price'] = $datum->pay_price;
            $newItem['mch_name'] = $datum->mch_name ? $datum['mch_name'] : '车海洋自营';//商户名称
            $newItem['pay_desc'] =  $datum->pay_desc ? $datum['pay_desc'] : '';
            $newItem['full_reduction_price'] = $datum->full_reduction_price ? $datum['full_reduction_price'] : '';
