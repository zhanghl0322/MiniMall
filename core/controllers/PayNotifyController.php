...
class PayNotifyController extends Controller
{
    ...
    private function wechatPayNotify($res)
    {
        ...
        $order->is_pay = 1;
        $order->pay_time = time();
        //$order->pay_type = 1;//TODO 微信支付成功会重置改类型、临时取消
        $order->is_cancel = 0;
        $order->is_delete = 0;
        if ($order->save()) {
            //支付完成之后，相关的操作
            ...
            //==============================================分割线========================================================
            if($order->pay_type==4)//账户+微信支付
            {
                //TODO 新增扣除记录  2019年8月19日14:15:32
                $log = new UserAccountLog();
                $log->user_id = $order->user_id;
                $log->type = 2;//1 增加  2支出
                $log->price = $user->money;
                $log->desc = "商城余额抵扣冻结，订单号为：{$order->order_no}。";
                $log->addtime = time();
                $log->order_id =$order->id;
                $log->order_type = 1;
                $log->save();

//                //TODO 提单临时扣除金额 置0  2019年8月19日14:11:34
//                $user_copy->money = 0;
//                $user_copy->save();
                \Yii::warning("商城余额抵扣冻结，订单号为：{$order->order_no}",'info');

                $user->money=0;
                $user->save();//支付成功清空订单余额
            }
            //===============================================分割线=======================================================
...
//        $this->payWriteBack_add($user->wechat_open_id,$user->wechat_union_id,$order->order_no,$order->id,$order->pay_price);
    ...
    public function unionOrderNotify($res)
    {
        ...
        foreach ($order_id_list as $order_id) {
            ...
            $order->is_pay = 1;
            $order->pay_time = time();
//            $order->pay_type = 1;  //屏蔽支付成功、订单类型重新赋值
            $order->is_cancel = 0;
            $order->is_delete = 0;
            if ($order->save()) {
                try {
                    ...
                    $log->type = 2;//1 增加  2支出
                    $log->price = $user->money;
                    $log->desc = "商城余额抵扣冻结，订单号为：{$order_union->order_no}。";
                    $log->addtime = time();
                    $log->order_id =$order->id;
                    $log->order_type = 1;
                    $log->save();
...
//                $user_copy->save();
     ...
     * 2018年5月3日11:37:08
     */
    public function payWriteBack_update($myOrderNO,$otherOrderNO)
    {
        //1.0 数据定义、
